...
void printResults(){
	...
	while(testCases--){
		cin >> userInput;
		if(userInput.size() == 5){
			if(isRightAlpha(userInput[0]) && isRightAlpha(userInput[3]) && isRightNumber(userInput[1]) && isRightNumber(userInput[4]) && userInput[2] == '-'){
				alphaDiff = userInput[3]-userInput[0];
				numbDiff = userInput[4]-userInput[1];
				if(alphaDiff > 0 && alphaDiff < 3 && numbDiff > 0 && numbDiff < 3){
					if(alphaDiff == 2){
						printf("%s\n",numbDiff == 1?"Yes":"No");
					}else{
						printf("%s\n",numbDiff == 2?"Yes":"No");
